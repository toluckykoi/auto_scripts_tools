...
```shell
# 快速更换系统镜像源:
curl -fsSL https://gitee.com/toluckykoi/auto_scripts_tools/raw/main/luckykoi_go.sh \
    -o luckykoi_go.sh \
 && chmod +x luckykoi_go.sh \
 && ./luckykoi_go.sh source
# 或
wget -qO luckykoi_go.sh https://gitee.com/toluckykoi/auto_scripts_tools/raw/main/luckykoi_go.sh && \
chmod +x luckykoi_go.sh && \
./luckykoi_go.sh source
...
+ 服务器巡检脚本实现
+ 各类软件安装：emqx、内网穿透、jupyter、nodejs、nvm、pm2、virtualenv、nvidia-container-toolkit、RustDesk
+ 实现的一键脚本：修改linux主机名、conda初始化、crontab定时任务、Python虚拟环境创建（支持venv、uv、conda、Virtualenv）、虚拟内存管理、系统信息查看、虚拟串口创建管理、网络优先级设置、debian sudo 一键初始化
+ 一键初始化 Linux 系统相关配置（新机开荒）
+ 一键更换 Linux 系统镜像源，加速软件安装

#### Python 相关：

+ 对 logging 库进行封装，方便其他调用
+ 对 mqtt 库进行二次封装，方便其他调用
+ 与Python相关的快捷处理脚本：初始化pip工具

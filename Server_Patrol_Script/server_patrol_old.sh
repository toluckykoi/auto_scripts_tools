...
# @Last Modified by:   蓝陌
# @Last Modified time: 2022-04-26 01:06:31
...
Protocol_Version=$(cat /etc/jiankongjioaben/sshd_config | grep Protocol | awk '{print $2}')
...
authorize_file=$(echo "$pwdfile" | grep -w $user | awk -F: '{printf $6"/.jiankongjioaben/authorized_keys"}')
...
echo "是否允许ROOT远程登录"
echo "--------------------"
config=$(cat /etc/jiankongjioaben/sshd_config | grep PermitRootLogin)
...
echo "--------------------"
cat /etc/jiankongjioaben/sshd_config | grep -v "^#" | sed '/^$/d'

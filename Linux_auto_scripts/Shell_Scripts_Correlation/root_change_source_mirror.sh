    ...
    if [ "$software_manager" == "apt" ] && [ "$ID" == "ubuntu" ]; then
        if [ "$VERSION_ID" == "16.04" ]; then
            ...
            if [ "$SYSTEM_ARCH" = "x86" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu/ubuntu16_sources.list /etc/apt/sources.list
            elif [ "$SYSTEM_ARCH" = "ARM" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu-ports/ubuntu16_sources.list /etc/apt/sources.list
            fi
            sed -i "/^[^#]/ s|https\?://[^/]*\(/ubuntu[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            ...
            echo "ubuntu 18.04"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
            if [ "$SYSTEM_ARCH" = "x86" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu/ubuntu18_sources.list /etc/apt/sources.list
            elif [ "$SYSTEM_ARCH" = "ARM" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu-ports/ubuntu18_sources.list /etc/apt/sources.list
            fi
             sed -i "/^[^#]/ s|https\?://[^/]*\(/ubuntu[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(ubuntu|security)"
            echo ""
            ...
            echo "ubuntu 20.04"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
            if [ "$SYSTEM_ARCH" = "x86" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu/ubuntu20_sources.list /etc/apt/sources.list
            elif [ "$SYSTEM_ARCH" = "ARM" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu-ports/ubuntu20_sources.list /etc/apt/sources.list
            fi
             sed -i "/^[^#]/ s|https\?://[^/]*\(/ubuntu[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(ubuntu|security)"
            echo ""
            ...
            echo "ubuntu 22.04"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
            if [ "$SYSTEM_ARCH" = "x86" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu/ubuntu22_sources.list /etc/apt/sources.list
            elif [ "$SYSTEM_ARCH" = "ARM" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu-ports/ubuntu22_sources.list /etc/apt/sources.list
            fi
             sed -i "/^[^#]/ s|https\?://[^/]*\(/ubuntu[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(ubuntu|security)"
            echo ""
            ...
            echo "ubuntu 24.04"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
            if [ "$SYSTEM_ARCH" = "x86" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu/ubuntu24_sources.list /etc/apt/sources.list
            elif [ "$SYSTEM_ARCH" = "ARM" ]; then
                 cp $DIR_PATH/ConfigFiles/linux/ubuntu-ports/ubuntu24_sources.list /etc/apt/sources.list
            fi
             sed -i "/^[^#]/ s|https\?://[^/]*\(/ubuntu[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(ubuntu|security)"
            echo ""
        ...
        if [ "$VERSION_ID" == "10" ]; then
            echo "debian 10"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
             cp $DIR_PATH/ConfigFiles/linux/debian/debian10_sources.list /etc/apt/sources.list
             sed -i "/^[^#]/ s|https\?://[^/]*\(/debian[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            ...
            echo "debian 11"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
             cp $DIR_PATH/ConfigFiles/linux/debian/debian11_sources.list /etc/apt/sources.list
             sed -i "/^[^#]/ s|https\?://[^/]*\(/debian[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(debian|security)"
            echo ""
             apt update
            ...
            echo "debian 12"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
             cp $DIR_PATH/ConfigFiles/linux/debian/debian12_sources.list /etc/apt/sources.list
             sed -i "/^[^#]/ s|https\?://[^/]*\(/debian[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(debian|security)"
            echo ""
             apt update
            ...
            echo "debian 13"
             cp /etc/apt/sources.list /etc/apt/sources.list.bak
             cp $DIR_PATH/ConfigFiles/linux/debian/debian13_sources.list /etc/apt/sources.list
             sed -i "/^[^#]/ s|https\?://[^/]*\(/debian[-/][^ ]*\)|$MIRROR_URL\1|g" "/etc/apt/sources.list"
            grep -v "^#" "/etc/apt/sources.list" | grep -E "(debian|security)"
            echo ""
             apt update
        ...
        if [ "$VERSION_ID" == "7" ]; then
            echo "centos 7"
             ...
             cp $DIR_PATH/ConfigFiles/linux/centos/huawei-CentOS-Base.repo /etc/yum.repos.d/CentOS-Base.repo
             yum clean all
             yum makecache
            echo "已更换镜像源"
        ...
        echo "fedora"
         cp -a /etc/yum.repos.d/fedora-updates.repo /etc/yum.repos.d/fedora-updates.repo.bak
         cp -a /etc/yum.repos.d/fedora.repo /etc/yum.repos.d/fedora.repo.bak

         cp $DIR_PATH/ConfigFiles/linux/fedora/fedora-updates.repo /etc/yum.repos.d/fedora-updates.repo
         cp $DIR_PATH/ConfigFiles/linux/fedora/fedora.repo /etc/yum.repos.d/fedora.repo
         sed -i "s|^\(baseurl=\s*\)https\?://[^/]*\(/fedora/[^\s]*\)|\1$MIRROR_URL\2|g" /etc/yum.repos.d/fedora-updates.repo
         sed -i "s|^\(baseurl=\s*\)https\?://[^/]*\(/fedora/[^\s]*\)|\1$MIRROR_URL\2|g" /etc/yum.repos.d/fedora.repo
